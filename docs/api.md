...
`apply=true` 且 dae 正在运行时，面板读取服务后端记录的主进程号（systemd 的 `MainPID`、procd 的实例 PID）并执行 `dae reload <PID>`，不依赖默认 PID 文件。dae 未运行时配置仍会保存成功，响应包含 `"deferred": true`，表示下次启动时读取；真正的 reload 错误仍会恢复旧磁盘配置并返回 `configuration_apply_failed`。
...
读取缓存、下载与校验不占用全局控制门，只有替换与按需重启阶段才进入串行区，避免几十兆的 I/O 把配置保存一并堵住。普通升级和切换仍需完整下载并校验上游发布包，但只解压可执行文件，不再浪费时间和内存解压首次安装物料；下载成功后会缓存二进制，后续切换直接使用本地版本。首次安装还需要 geo 数据（systemd 后端下还需要用来渲染服务单元的模板），因此即使该版本已有二进制缓存，也会重新取得并校验完整发布包。种子配置由面板内置，不依赖上游是否附带 `empty.dae`。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/connections?limit=500` | 连接活动记录与存活判定，`limit` 裁剪响应条数（1–2000，默认 500） |

裁剪时存活与孤儿记录优先占用配额（否则"存活中"视图会残缺），但不豁免上限：存活记录本身
超过 `limit` 时只保留较新的部分。有记录未被逐条列出时响应带 `truncated: true`，`summary`
里的计数不受裁剪影响、始终是完整值。单次快照记录的入站腿另有 5000 条硬上限，同样反映在
`truncated` 上——记录条数由局域网流量决定，不设闸等于把响应大小交给外部控制。

数据全部来自公开接口，与本面板的零侵入约束一致：元数据取自 dae 在 `info` 级别为每条连接输出的
建立日志（出站、节点、嗅探域名、进程名、MAC…），存活证据取自 `/proc/net/tcp{,6}` 中 dae 进程
持有的 socket。dae 是 tproxy 透明代理、不做 NAT，它接受的入站 socket 在内核表里恰好是
`local=原始目的、remote=客户端`，与日志四元组逐项对得上——对账由此成立，不读 dae 内部 eBPF Map。

每条记录的 `status` 有四种：`live`（四元组有对应 socket）、`closed`（socket 已消失）、
`unknown`（判不了：UDP 没有关闭语义；被 eBPF 卸载的 TCP 未命中 socket 时宁可说不知道；
快照采集失败时全体降级）、`orphan`（socket 存活但建立日志已滚出窗口，只有裸四元组，
`firstSeen` 是面板首次观测时刻，`approxFirstSeen` 为 `true`）。

已知边界，前端文案与本表保持一致：

| 边界 | 行为 |
|---|---|
| dae 日志级别高于 `info` | 采集不到新事件，响应带 `logLevel` 供前端给出配置引导；存活 socket 仍以 `orphan` 列出 |
| 日志缓冲区窗口有限（OpenWrt 默认 64 KiB） | 面板在内存里累积事件（上限 2000 条、24 小时，重启清零），把窗口从缓冲区寿命延长到面板运行期 |
| 上游日志格式变化 | 解析失败的行静默跳过，只累计到 `dropped`；页面据此提示而不是报错 |
| 日志内容可被外部影响 | 嗅探到的域名、订阅里的节点名都可能含引号。解析器按 logrus 的 `%q` 规则处理转义并对同名键取首次出现的值，避免日志内容伪造出 `outbound` 一类字段；`network` 只接受 dae 的四种取值 |
| 每连接流量、速率、时长精确值与关闭操作 | 不提供：需要读 dae 内部状态，超出零侵入边界 |
