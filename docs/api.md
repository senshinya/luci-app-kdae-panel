...
读取缓存、下载与校验不占用全局控制门，只有替换与重启阶段才进入串行区，避免几十兆的 I/O 把配置保存一并堵住。普通升级和切换只缓存可执行文件；首次安装还需要 geo 数据（systemd 后端下还需要用来渲染服务单元的模板），因此即使该版本已有二进制缓存，也会重新取得并校验完整发布包。种子配置由面板内置，不依赖上游是否附带 `empty.dae`。
