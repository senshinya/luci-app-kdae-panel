...
- 结构化日志浏览、搜索和级别筛选；
- 连接活动面板：把 dae info 级别的连接建立日志与 `/proc/net` 里 dae 进程持有的 socket 按四元组对账，得到"存活中 / 全部活动"两个视图，展示源、目的、嗅探域名、出站、节点、进程与 MAC，支持按活跃时间双向排序；UDP 与被 eBPF 卸载的连接如实标为不可判定，日志已滚出窗口的存活 socket 以孤儿形式列出；不读 dae 内部 eBPF Map，因此不提供每连接流量、速率与关闭操作；
