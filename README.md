...
- 节点选择页：把 `policy: fixed(n)` 分组的候选点选成瓦片，点一下即可切到该节点并对 dae 完成无损重载；只有当分组的候选节点全部落在同一个 dae tag（本地 `node` 节或某一份订阅）、且能与该 tag 的列表逐一对齐时才铺出瓦片，因为 `fixed(n)` 的 `n` 是 dae 内部按 tag 遍历生成的下标，跨 tag 的顺序每次启动和重载都可能变化；条件不满足的分组只给按索引切换的输入框，并说明具体原因，不展示面板确认不了的节点名；
...
- 结构化日志浏览、搜索和精确级别筛选；页面同时显示并可修改 dae 实际输出级别，修改仍经过配置校验、原子保存与 reload；
